...
def _train_by_episode(n_episodes, ):
    flag_storing = 1
    for episode in range(n_episodes):
        ...
        if flag_storing and episode % 100 == 0:
            print('data collection: {} ,buffer capacity: {} '.format(episode / 100,
                                                                     len(train_agent.replay_buffer)))
        ...
        state_train_agent, state_use_agent = alloc.env_reset(env, train_agent_name)
        while True:
            # give the action based on models
            action_train_agent = train_agent.egreedy_action(state_train_agent)
            action_use_agent = use_agent.max_action(state_use_agent)
            ...
            e_reward += reward
            step += 1
            train_agent.store_data(state_train_agent, action_train_agent, reward_train_agent, next_state_train_agent,
                                   done)
            # 当储存样本数到达指定数目时，则开始训练
            if (len(train_agent.replay_buffer) >= 100000):
                flag_storing = 0
